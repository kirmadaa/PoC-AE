...
def main():
    ...
    for tag in newer_tags:
        ...
        built_test_image = f"{args.image}-test-{tag}"
        
        print(f"Testing tag: {tag}")
        try:
            # First scan the base image
            base_vulns = scan_image(base_test_image)
            base_high_critical, base_total = print_vulnerability_summary(f"{tag} (base)", base_vulns)
            
            # Now build our actual application on this base image and scan it
            print(f"  Building test image with {tag}...")
            
            # Temporarily update dockerfile to use this tag
            temp_dockerfile = args.dockerfile + f".temp-{tag}"
            ...
            try:
                ...
                run_cmd(["docker", "build", "-f", temp_dockerfile, "-t", built_test_image, args.build_context])
                
                # Scan the built image (this is what actually matters!)
                built_vulns = scan_image(built_test_image)
                built_high_critical, built_total = print_vulnerability_summary(f"{tag} (built)", built_vulns)
                
                tag_results.append({
                    'tag': tag,
                    'base_high_critical': base_high_critical,
                    'base_total_vulns': base_total,
                    'built_high_critical': built_high_critical,
                    'built_total_vulns': built_total,
                    'base_vulns': base_vulns,
                    'built_vulns': built_vulns
                })
                
                # Clean up temporary files and images
                os.remove(temp_dockerfile)
                subprocess.run(["docker", "rmi", built_test_image], capture_output=True)
                
            except subprocess.CalledProcessError as build_error:
                print(f"  Failed to build with {tag}, skipping...")
                if os.path.exists(temp_dockerfile):
                    os.remove(temp_dockerfile)
                continue
                
        except subprocess.CalledProcessError:
            print(f"Failed to scan base image {base_test_image}, skipping...")
            continue
    ...
    if tag_results:
        ...
        # Sort by BUILT image HIGH/CRITICAL first (ascending), then by built total vulnerabilities (ascending)
        tag_results.sort(key=lambda x: (x['built_high_critical'], x['built_total_vulns']))
        
        print("\nRanked results (based on BUILT images, best to worst):")
        for i, result in enumerate(tag_results, 1):
            print(f"{i}. {result['tag']} - Built: HIGH/CRITICAL: {result['built_high_critical']}, Total: {result['built_total_vulns']} "
                  f"| Base: HIGH/CRITICAL: {result['base_high_critical']}, Total: {result['base_total_vulns']}")
        
        # Pick the best tag based on BUILT image results
        best_result = tag_results[0]
        if (best_result['built_high_critical'] < best_vulns or 
            (best_result['built_high_critical'] == best_vulns and best_result['built_total_vulns'] < best_total_vulns)):
            best_tag = best_result['tag']
            best_vulns = best_result['built_high_critical']
            best_vuln_list = best_result['built_vulns']
            best_total_vulns = best_result['built_total_vulns']
            print(f"\nSelected best tag: {best_tag} (Built image will have {best_vulns} HIGH/CRITICAL, {best_total_vulns} total)")
        else:
    ...
    print_vulnerability_summary("final built image", final_vulns)
